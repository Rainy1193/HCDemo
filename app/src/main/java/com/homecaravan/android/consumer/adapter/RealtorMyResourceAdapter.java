...
public class RealtorMyResourceAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private Context mContext;
    private boolean mBorderAvatarImage;
    private FragmentTeamChild fragmentTeamChild;
    private ArrayList<ConsumerTeam> mArrTeam;

    public RealtorMyResourceAdapter(Context mContext, boolean mBorderAvatarImage, FragmentTeamChild fragmentTeamChild, ArrayList<ConsumerTeam> mArrTeam) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        vh = new RealtorHolder(v);
        return vh;
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        final ConsumerTeam realtor = mArrTeam.get(position);
        RealtorHolder realtorHolder = (RealtorHolder) holder;

        realtorHolder.mLayoutItem.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ...
                tvSetExclusive.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        fragmentTeamChild.showSelectedRealtor(realtor);
                        alertDialog.dismiss();
                    }
                });
        ...
        if (!mBorderAvatarImage) {
            realtorHolder.vImgAvatarRealtor.setBorderWidth(R.dimen.non_border_width);
        }


        Glide.with(mContext.getApplicationContext()).load(realtor.getPhoto()).asBitmap().fitCenter()
                .placeholder(R.drawable.no_image)
                .dontAnimate()
                .into(realtorHolder.vImgAvatarRealtor);

        Glide.with(mContext.getApplicationContext()).load(realtor.getLogo()).asBitmap().fitCenter()
                .placeholder(R.drawable.no_image)
                .dontAnimate()
                .into(realtorHolder.mImgLogoCompany);

        realtorHolder.mTvName.setText(realtor.getFirstName() + " " + realtor.getLastName());

        realtorHolder.mTvJobs.setText(realtor.getCompany());

        if(realtor.getStatus().equals("active")){
            realtorHolder.mVStatus.setBackgroundResource(R.drawable.dot_green_online);
        } else{
            realtorHolder.mVStatus.setBackgroundResource(R.drawable.dot_gray_offline);
        }
    }
    ...
    public class RealtorHolder extends RecyclerView.ViewHolder {
