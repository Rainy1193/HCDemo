...
/**
 * Copyright chensuilun at https://github.com/BCsl/GalleryLayoutManager
 ...
 * support both {@link CustomLayoutManager#HORIZONTAL} and {@link CustomLayoutManager#VERTICAL} scroll.
 */
public class CustomLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private boolean mNoAnimationScroll;
    ...
    public void setNoAnimationScroll(boolean mNoAnimationScroll) {
        this.mNoAnimationScroll = mNoAnimationScroll;
    }
    ...
    public RecyclerView.LayoutParams generateDefaultLayoutParams() {
        if (mOrientation == VERTICAL) {
            return new CustomLayoutManager.LayoutParams(
    ...
    /**
     * Layout the item view witch position specified by {@link CustomLayoutManager#mInitialSelectedPosition} first and then layout the other
     *
     * @param recycler
     * @param state
     */
    ...
    /**
     * Layout the item view witch position special by {@link CustomLayoutManager#mInitialSelectedPosition} first and then layout the other
     *
     * @param recycler
     * @param state
     */
    ...
    private float calculateToCenterFraction(View child, float pendingOffset) {
        ...
        int childLength = mOrientation == CustomLayoutManager.HORIZONTAL ? child.getWidth() : child.getHeight();
        childLength = childLength / 2;// change speed scroll item
    ...
    private int calculateDistanceCenter(View child, float pendingOffset) {
        ...
        if (mOrientation == CustomLayoutManager.HORIZONTAL) {
    ...
    public interface ItemTransformer {
         ...
         */
        void transformItem(CustomLayoutManager layoutManager, View item, float fraction);
    }
    ...
    /**
     * Implement to support {@link CustomLayoutManager#smoothScrollToPosition(RecyclerView, RecyclerView.State, int)}
     */
    private class GallerySmoothScroller extends LinearSmoothScroller {
        ...
        protected void onTargetFound(View targetView, RecyclerView.State state, Action action) {
            ...
            distance = distance * 2; // set speed smooth scroll position
            final int time = calculateTimeForDeceleration(distance);
            if (time > 0) {
                if (mNoAnimationScroll) {
                    action.update(-dx, -dy, 1, mDecelerateInterpolator);// time=1
                } else {
